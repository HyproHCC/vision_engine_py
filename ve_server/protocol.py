...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        if ".." in img_path:
            raise ProtocolError(E_BAD_FIELD, "directory traversal sequence '..' not allowed in image_path")
        if not img_path.lower().endswith(ALLOWED_IMAGE_EXTS):
            raise ProtocolError(E_BAD_FIELD, "image_path extension not allowed")
