...
VALID_IMAGE_EXTENSIONS = (".png", ".bmp", ".jpg", ".jpeg", ".tif", ".tiff")
...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        if ".." in img_path.replace("\\", "/"):
            raise ProtocolError(E_BAD_FIELD, "path traversal sequence in image_path")
        if not img_path.lower().endswith(VALID_IMAGE_EXTENSIONS):
            raise ProtocolError(E_BAD_FIELD, "unsupported file extension in image_path")
