...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        # 阻擋路徑穿越 (Path Traversal) 攻擊
        if ".." in image_path:
            raise ProtocolError(E_BAD_FIELD, "image_path contains directory traversal sequences")

        # 限制支援的圖片格式副檔名
        _, ext = os.path.splitext(image_path.lower())
        if ext not in (".png", ".bmp", ".jpg", ".jpeg", ".tif", ".tiff"):
            raise ProtocolError(E_BAD_FIELD, "unsupported image format extension")
