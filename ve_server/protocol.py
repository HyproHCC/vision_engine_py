...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        if ".." in norm_path.split("/"):
            raise ProtocolError(
                E_BAD_FIELD,
                "directory traversal sequence '..' not allowed in image_path")
        if not norm_path.lower().endswith(VALID_IMAGE_EXTS):
            raise ProtocolError(
                E_BAD_FIELD,
                "invalid image_path extension, allowed: %s" % ", ".join(VALID_IMAGE_EXTS))
