...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        # Security validation: path traversal prevention & image extension whitelist
        if ".." in img_path:
            raise ProtocolError(E_BAD_FIELD, "path traversal sequence in image_path is forbidden")
        allowed_exts = (".png", ".bmp", ".jpg", ".jpeg", ".tif", ".tiff")
        if not img_path.lower().endswith(allowed_exts):
            raise ProtocolError(E_BAD_FIELD, "unsupported image file extension in image_path")
