...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        ...
        # Security check: Prevent directory traversal (e.g., contains '..')
        if ".." in image_path:
            raise ProtocolError(E_BAD_FIELD, "directory traversal sequence '..' is not allowed in image_path")

        # Security check: Restrict file extension to allowed image formats
        import os
        _, ext = os.path.splitext(image_path.lower())
        if ext not in (".png", ".bmp", ".jpg", ".jpeg", ".tif", ".tiff"):
            raise ProtocolError(E_BAD_FIELD, "unsupported or missing image file extension in image_path")
