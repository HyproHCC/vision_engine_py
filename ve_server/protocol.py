...
def parse_request(line: str) -> dict:
    ...
    if cmd in ("inspect", "teach"):
        img_p = req.get("image_path")
        if not isinstance(img_p, str) or not img_p:
            raise ProtocolError(E_BAD_FIELD, "missing image_path")
        if ".." in img_p.replace("\\", "/").split("/"):
            raise ProtocolError(E_BAD_FIELD, "image_path contains path traversal sequence '..'")
        if not img_p.lower().endswith(ALLOWED_IMAGE_EXTS):
            raise ProtocolError(E_BAD_FIELD, "unsupported image extension")
